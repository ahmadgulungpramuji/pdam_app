...
class ApiService {
  ...
  String get rootBaseUrl { //
    ...
    if (baseUrl.endsWith('/api/')) { // jaga-jaga jika ada trailing slash setelah /api
      return baseUrl.substring(0, baseUrl.length - '/api/'.length); //
    }
    return baseUrl; //
  }
  ...
  Future<Map<String, dynamic>> registerCalonPelanggan({ //
    ...
    var request = http.MultipartRequest('POST', url); // DIUBAH: Registrasi seharusnya POST (membuat data baru)
  ...
  Future<Map<String, dynamic>?> updateUserProfile( //
    ...
    if (token == null) { //
      ...
      throw Exception('Autentikasi diperlukan. Silakan login kembali.'); // <--- LEMPAR EXCEPTION INI
    }
    ...
    try { //
      if (profileImage != null) { // Jika ada gambar, gunakan MultipartRequest (POST + _method: 'PATCH')
        // === PERUBAHAN KUNCI 1 ===
        ...
        updatedData.forEach((key, value) { // Tambahkan field data teks
          request.fields[key] = value.toString(); //
        });

        request.files.add( // Tambahkan file gambar
          await http.MultipartFile.fromPath( //
        ...
        final streamedResponse = await request.send().timeout(const Duration(seconds: 60)); // Tambah timeout
        response = await http.Response.fromStream(streamedResponse); //
      } else { // Jika tidak ada gambar, gunakan PATCH untuk data JSON saja
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) { 
        ...
        final errorResponse = jsonDecode(response.body); //
        throw Exception( //
          errorResponse['message'] ?? 'Gagal memperbarui profil (Status: ${response.statusCode})', //
        );
      }
    } on TimeoutException { //
      ...
      throw Exception('Terjadi kesalahan jaringan atau sistem: ${e.toString()}'); //
    }
  }
  ...
  Future<Map<String, dynamic>> updateStatusTugas({ //
    ...
    if (keterangan != null && keterangan.isNotEmpty) { // <--- KIRIM KETERANGAN JIKA ADA
    ...
    if (response.statusCode == 200 || response.statusCode == 201) { //
      ...
      throw Exception( //
        errorBody['message'] ?? 'Gagal update status tugas: ${response.body}', // Tampilkan pesan dari backend
      );
    }
  }
